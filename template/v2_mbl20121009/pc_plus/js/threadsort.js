...
function showerror(c,k){	
	$(c).html('<i class="data_invalid"></i><span>'+k+'</span>'); 
	}

function showtrue(c){
	$(c).html('<i class="data_valid"></i>');
	}
...
function checkoption(identifier, required, checktype, checkmaxnum, checkminnum, checkmaxlength) {
	...
	if(checktype == 'select') {
		if(required != '0' && ($$('typeoption_' + identifier) == null || $$('typeoption_' + identifier).value == '0')) {
			...
			ce.innerHTML = '<i class="data_invalid"></i>';			
	...
	if(required != '0') {
		if(checkvalue == '') {
			...
			ce.innerHTML = '<i class="data_valid"></i>';
		}
	}
